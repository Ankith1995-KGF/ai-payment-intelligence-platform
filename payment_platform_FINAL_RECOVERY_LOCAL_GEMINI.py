...
def initialize_gemini_from_environment(env_name="GEMINI_API_KEY"):
    """Initialize Gemini from a normal OS environment variable."""
    global client
    try:
        from google import genai
        ...
        if not api_key:
            ...
            return {
                "status": "not_initialized",
                "reason": f"{env_name} environment variable not found"
            }
        client = genai.Client(api_key=api_key)
        return {"status": "initialized", "source": env_name}
    except Exception as error:
        client = None
        return {"status": "not_initialized", "reason": str(error)}


def ensure_gemini_client():
    """Lazily initialize Gemini once for local/Streamlit execution."""
    global client
    if client is None:
        initialize_gemini_from_environment()
    return client
...
def run_fraud_sandbox(
    ...
    # --------------------------------------------------
    # 4A. CONTEXTUAL CNP CALIBRATION
    # --------------------------------------------------
    # Preserve the trained XGBoost model, but avoid treating CNP as a
    # standalone fraud verdict. A modest calibration is applied only when
    # the transaction also shows legitimate behavioural evidence.
    raw_model_risk_score = risk_score
    cnp_calibration = 0.0

    # Use the feature row already built for this transaction.
    # `behavior` exists inside build_sandbox_model_features(), not in this function.
    feature_row = sandbox_model_features.iloc[0]

    if str(channel).upper() == "CNP":
        if feature_row.get("new_device", 1) == 0:
            cnp_calibration += 0.10
        if feature_row.get("previous_device_uses", 0) >= 3:
            cnp_calibration += 0.06
        if feature_row.get("new_country", 1) == 0:
            cnp_calibration += 0.05
        if feature_row.get("unusual_transaction_hour", 1) == 0:
            cnp_calibration += 0.03
        if feature_row.get("amount_vs_customer_avg_filled", 99) <= 1.5:
            cnp_calibration += 0.08

        # Cap the adjustment so genuinely high-risk CNP transactions remain risky.
        cnp_calibration = min(cnp_calibration, 0.24)
        risk_score = max(0.0, risk_score - cnp_calibration)
...
def call_gemini_with_retry(
    ...
    gemini_client = ensure_gemini_client()
    ...
    if gemini_client is None:
        ...
        return {
            "status": "fallback",
            "source": "Deterministic Fraud Engine",
            "text": fallback_text
        }
...
def compare_customer_currency_options(
    ...
    def customer_fx_cost(selected_currency):
        # No cardholder FX conversion when paying in the customer's home currency.
        if selected_currency == customer_currency:
            return 0.0

        base_pair_markup = float(
            fx_pair_markup.get(
                (customer_currency, selected_currency),
                0.015
            )
        )

        # Project assumption: INR cardholders incur an additional foreign-currency
        # conversion/issuer markup when choosing a non-INR billing currency.
        # This is deliberately an assumption, not a claim about any specific bank.
        issuer_foreign_currency_markup = (
            0.035 if customer_currency == "INR" else 0.020
        )

        return base_pair_markup + issuer_foreign_currency_markup

    comparison[
        "customer_fx_markup_rate"
    ] = comparison[
        "currency"
    ].apply(customer_fx_cost)
...
def call_currency_gemini_with_retry(
    ...
    if gemini_client is None:
        recommended_currency = evidence["recommended_currency"]
        customer_currency = evidence["customer_home_currency"]
        merchant_currency = evidence["merchant_home_currency"]
        estimated_cost = evidence["estimated_total_cost_inr"]
        savings = evidence["savings_vs_next_best_inr"]
...
used here are project assumptions and are not actual issuer,
merchant, Mastercard, Visa, or market pricing.
""".strip()

        return {
            "status": "fallback",
            "source": "Deterministic Currency Engine",
            "text": fallback_text
        }
